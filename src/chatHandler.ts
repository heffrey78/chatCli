...
async function handlePrompt(prompt: string, messages: IMessage[]): Promise<boolean | undefined> {
    const parsedPrompt = parsePrompt(prompt);
    const commandName = parsedPrompt ? parsedPrompt.command.replace(/[\r\n]/gm, '').toUpperCase() : null;
...
function parsePrompt(prompt: string): ParsedPrompt | undefined {
  ...
  function systemMessageExists(messages: IMessage[]) : boolean {
    return messages.findIndex((message) => message.role === "system") === 0;
  }

  function addSystemMessage(messages: IMessage[]) : void {
